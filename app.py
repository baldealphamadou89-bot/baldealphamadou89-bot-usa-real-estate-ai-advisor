...
with st.sidebar:
    ...
    if not gemini_key:
        gemini_key = st.text_input("Entrez votre Gemini API Key", type="password")
    else:
        st.success("✅ Clé Gemini chargée via Secrets")
        
    if not maps_key:
        maps_key = st.text_input("Entrez Google Maps API Key (Optionnel)", type="password")
    else:
                ...
                with col2:
                    ...
                    if maps_key:
                        ...
                        if img:
                            ...
                            vision_prompt = "Analyse l'état visuel du toit, des fenêtres et de la façade. Y a-t-il des signes de dommages ou d'abandon ?"
