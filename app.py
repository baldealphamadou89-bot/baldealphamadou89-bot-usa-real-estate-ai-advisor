...
st.markdown("""
    ...
    .external-link:hover { background-color: #ffebee; border-color: #b71c1c; transform: scale(1.02); }
...
with st.sidebar:
    ...
    # Conteneur dédié aux liens pour assurer la visibilité
    comps_container = st.container()
    
    st.markdown(f'<p style="font-size: 0.85em; color: #d32f2f; text-align: center; font-weight: bold; margin-top: 20px;">{t["exp_text"]}</p>', unsafe_allow_html=True)
...
if uploaded_file:
    if "OPENAI_API_KEY" not in st.secrets:
        ...
        # --- GÉNÉRATION DES LIENS DANS LA SIDEBAR ---
